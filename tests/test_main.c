...
int parcours_fat_print(unsigned char* aes_key, block_t directory_block, uint32_t directory_index)
{
    printf("START:");
    ...
    int count = 1;
    printf("count=%u FAT[%lu]=%u\n", count, block, read_fat_offset(aes_key, block));
...
int main(void)
{
    ...
    set_device_path(
        "tests/entry_write_buffer_from.test.shlkfs");
    ...
    entry_read_raw_data(ase_key, TEST_entry.start_block, 0, 0, buff1, strlen(path));
    assert(res == 0);
    ...
    // read_blocks_with_decryption(ase_key, dir->entries[0].start_block, 1, dir);    
