...
int cryptfs_getattr(const char *path, struct stat *stbuf)
{
    ...
    printf("used %i\ntype %i\n start_block %lu\nname %s\n size %lu\n uid %u\ngid %u\nmode %u\natime %u\nmtime %u\nctime %u\n",
           entry->used, entry->type, entry->start_block, entry->name, entry->size, entry->uid, entry->gid, entry->mode, entry->atime, entry->mtime, entry->ctime);
...
int cryptfs_read(const char *path, char *buf, size_t sz, off_t offset,
    ...
    byte_read = entry_read_raw_data(fuse_info.aes_key, entry_id, offset, buf, sz);
